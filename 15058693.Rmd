...
author: Student id 15058693, candidate nr FDWD6
...
---
#This is student 15058693 code submission for the Data Science Project. The structure follows: loading libraries, importing data, modelling linguistic properties/stylometric analysis, classification and training, results, and the code for (web) data collection is pasted in the very final section to not confuse with the rest of the analysis.
...
# Here I begin modelling the linguistic properties of the text/stylometric analysis of the text. I will look at number of sentences, number of sentences per author, average sentence length, number of words, distribution of number of words, number of words by author, word length, and vocabulary size to see if there are any interesting findings.
...
```
# I was curious to see the distribution and surprised to see that it is not normally distributed, but rather follows a multimodal distribution.
...
### Now I will look at average number of words by author
...
## number of commas
### Here I look at average number of commas by author, just too identify if there are any more specific features that could seperate them apart. First one is distribution
...
pander(tstat_freq)
```
From observation, there are a few overlaps like "can" and "on". Interesting to see "women" and "kids" being top 8 for two female authors. Overall quite rich diversity.. Stop words have been removed which could explain why it doesn't follow common english words
...
The model has an accuarcy of 0.826 (82.60%) on the testing data, which is overall  very good to identify these authors based on text data specifically. As seen, looking at statistics by class, some report very high numbers. The model in good in identifying author based on text data, but not perfect. See results and discussion in the report.

# DATA COLLECTION / ANNEX
# Here is the code used for gathering the data. Libraries used writexl, jsonlite, rvest, stringr
#I have pasted the code in one section to make it easy to retrieve all data at the same time
